  ...
  return (
    <>
      <button
        ...
        className="fixed top-1/2 -translate-y-1/2 right-0 z-40 bg-[var(--color-coral)] hover:bg-[var(--color-coral-600)] text-white font-semibold tracking-wider px-3 py-4 rounded-l-xl shadow-lg"
      ...
      {open && (
        <div className="fixed inset-0 z-50">
          ...
          <aside
            ...
            aria-modal="true"
            aria-label="Cargá tu CV"
            className="absolute right-0 top-0 h-full w-full sm:w-[460px] bg-white shadow-2xl flex flex-col"
          >
...
function Field({ name, label, type = "text" }: { name: string; label: string; type?: string }) {
  return (
    <div>
      ...
      <input id={`drw-${name}`} name={name} type={type} required className="w-full rounded-xl border border-[var(--color-petroleo-100)] bg-white px-3 py-2.5" />
    </div>
  );
}
