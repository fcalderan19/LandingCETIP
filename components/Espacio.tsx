    ...
    <section id="espacio" className="py-16 md:py-24">
        ...
        <div className="mt-10 grid grid-cols-2 md:grid-cols-4 grid-rows-[200px_200px] md:grid-rows-[260px_260px] gap-3 reveal">
          {fotos.map((f, i) => (
            <button
              ...
              className={`relative overflow-hidden rounded-2xl group shadow-sm hover:shadow-xl transition-shadow ${f.h}`}
            >
              <img src={f.src} alt={f.alt} loading="lazy" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
      ...
      {lightbox !== null && (
        <div
          ...
          aria-modal="true"
        >
          <button className="absolute top-4 right-4 text-white hover:rotate-90 transition-transform" aria-label="Cerrar">
            <IconClose width={28} height={28} />
          </button>
          <img src={fotos[lightbox].src} alt={fotos[lightbox].alt} className="max-h-[85vh] max-w-[95vw] rounded-xl shadow-2xl anim-pop" />
        </div>
      )}
    </section>
  );
