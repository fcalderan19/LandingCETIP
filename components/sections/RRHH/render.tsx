...
import type { RRHHProps } from "./schema";

export default function RRHHRender({
  eyebrow,
  title,
  body,
  bullets,
    ...
    >
      <div className="mx-auto max-w-7xl px-4 grid md:grid-cols-2 gap-12 items-start">
        <div className="reveal">
          ...
          <p className="mt-4 text-[var(--color-petroleo)]/80">{body}</p>
          {bullets.length > 0 && (
        ...
        >
          ...
          <div>
            <label
              htmlFor="area_interes"
              className="block text-sm font-medium text-[var(--color-petroleo)] mb-1"
            >
          ...
          {state === "ok" && (
            <p className="text-sm text-[var(--color-verde-600)] font-medium">
              ¡Gracias! Recibimos tu postulación.
            </p>
          )}
          {state === "error" && (
            <p className="text-sm text-[var(--color-coral)] font-medium">
              No pudimos enviar. Intentá nuevamente.
            </p>
          )}
        </form>
      </div>
